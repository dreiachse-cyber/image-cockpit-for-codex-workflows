...
function workflowNotes(mode: CodexWorkflowMode) {
  ...
  if (mode === "sprite-generate") {
    return [
      ...
      "For grounded animation presets, keep the supporting foot or foot silhouette on a consistent visual ground line in every contact frame; for run/recoil frames, allow airborne beats but avoid vertical foot-anchor drift.",
...
function buildCodexRunnerPrompt(job: { id: string; path: string; outboxDir: string }) {
  return [
    ...
    "For standard direction-split grounded presets, self-check the visual ground line before publishing: contact frames need foot or foot-silhouette pixels in the same ground band, and soft/run presets may include airborne frames only if the row does not drift vertically.",
