  ...
  if (mode === "sprite-generate") {
    return [
      ...
      backgroundMode === "direct-transparent"
        ? "Use direct transparent output: every returned direction image must be a PNG with a real alpha channel, empty transparent alpha outside the character, and no drawn background, preview pattern, guide grid, floor, or backdrop. Preserve the source character's full-color pixel art, original palette, outfit or body markings, equipment, props, outlines, and shading; do not turn the character into a black silhouette, shadow, mask, monochrome cutout, or flat color stamp. If true alpha cannot be produced, do not mark the result successful as a chroma-key fallback."
