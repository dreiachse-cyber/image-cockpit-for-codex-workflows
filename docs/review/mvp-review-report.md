...
このレビュー記録は、Codexがインストールされたローカル環境で、画像生成 / 画像編集 / アニメーション生成を一つの制作コクピットから扱えるかを確認するためのv0.1.0公開前レビュー履歴です。
...
2. 表示されたlocal URLを開く。
3. ワークフロー欄で Pixel Art Generation / Image Editing / Animation Generation の3タブを確認する。
4. `画像編集` を開き、prompt、編集メモ、注釈、選択画像がCodex handoff jobへ入ることを確認する。
5. `アニメーション生成` でsource選択、プリセット選択、sprite sheet / GIF / WebP exportを確認する。
...
- 日本語 / English の言語切替で、主要workflowのパネル、canvas/grid、sprite、export周辺の中核ラベルが追従する。
...
- release auditで、主要workflowの中核UIラベルと初期言語判定が言語切替の対象になっていることを確認。
- `npm run ui:smoke` で、3つのworkspace tab、言語切替、主要workflowそれぞれのroute、主ボタン、download flowがheadless browser上で見えることを確認。
...
- repoはv0.1.0でpublic化済み。以後のvisibility / branch protection / release変更はご主人確認後。
