...
1. ご主人レビュー: `docs/release/v0.1.0-owner-review.md`、`docs/release/v0.1.0-final-audit.md`、`docs/review/mvp-review-report.md`、`docs/release/v0.1.0-owner-decision.md` の順番でprivate MVPを触って確認する
2. レビューで出たP0/P1/P2を直す
3. 選択されたCodex launch commandで、実際の `codex exec` no-image job自動完走は確認済み。画像生成/編集可否はCodex環境ごとに確認する
...
4. `Codex Handoff` でjobを作り、`codex-handoff/inbox/*.json` を確認する
5. `codex exec` が使える環境ではrunner status/logsを確認する。使えない環境では画像を `codex-handoff/outbox/` に置き、`Local Inbox` から取り込めるか見る
6. 取り込んだ画像をgrid splitして、PNG / ZIP / GIF / metadataを書き出す
7. ここまでで「初回MVPとして見せてよいか」を判断する
