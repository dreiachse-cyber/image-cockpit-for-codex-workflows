...
- Automatic `codex exec` completion must still be verified on at least one environment where the selected Codex launch command can run a real job, or the first release must explicitly ship as manual-handoff-first.
