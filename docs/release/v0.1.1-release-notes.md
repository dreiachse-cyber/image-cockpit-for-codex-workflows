...
Status: draft for v0.1.1 tag approval
...
Feature scope for this draft is the current main stabilization range after v0.1.0, ending at the Image Editing full-body fit safeguards. The dev-supervisor repair and bronze-candidate duplicate import work tracked in 027/028 is not included in this draft unless it is merged before the final tag decision.
...
- Added QA documentation for job concurrency, Codex log behavior, generation reliability, artifact staging, storage recovery, image edit full-body fitting, and temporary artifact filtering.
