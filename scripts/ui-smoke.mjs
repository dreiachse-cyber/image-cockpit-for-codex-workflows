...
async function assertHistoryIncrementalRendering() {
  ...
  snapshot = await pageSnapshot();
  const historyTotalAfterScroll = Number(snapshot.historyTotalCount);
  assert(
    snapshot.historyItems >= 120 && snapshot.historyItems <= historyTotalAfterScroll,
    `Results list should render at least 120 cards without exceeding the current total ${historyTotalAfterScroll}, got ${snapshot.historyItems}`
  );

  if (snapshot.historyItems < historyTotalAfterScroll) {
    await scrollHistoryListToBottom();
    await waitForEval(
      () => `document.querySelector(".history-list")?.dataset.visibleCount === document.querySelector(".history-list")?.dataset.totalCount`,
      "Results list caps at all available cards"
    );
  }
  snapshot = await pageSnapshot();
  assert(
    snapshot.historyItems === Number(snapshot.historyTotalCount),
    `Results list should render all ${snapshot.historyTotalCount} available cards after the final scroll, got ${snapshot.historyItems}`
  );
